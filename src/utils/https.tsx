...
export const deletePizza = async ({id}: {id:number}):Promise<void> => {
    ...
    if(!response.ok) {
        const error:ErrorInterface = new Error('Pojawil sie problem podczas usuwania pizzy');
        error.code = response.status;
        error.info = await response.json();
        throw error
    }
...
export const addPizza = async (pizzaData:NewPizzaObj):Promise<void> => {
    ...
    if(!response.ok) {
        const error:ErrorInterface = new Error('Podczas dodawania pizzy wystapil blad');
        error.code = response.status;
        error.info = await response.json();
        throw error;
    }
...
export const addPizzaToCart = async ({id}:{id:number}):Promise<void> => {
    ...
    if(!response.ok) {
        const error:ErrorInterface = new Error('Podczas dodawania pizzy do koszyka wystapil problem');
        error.code = response.status;
        error.info = await response.json();
        throw error;
    }
...
export const removePizzaFromCart = async ({id}:{id:number}):Promise<void> => {
    ...
    if(!response.ok) {
        const error:ErrorInterface = new Error('Podczas usuwania pizzy z koszyka wystapil problem');
        error.code = response.status;
        error.info = await response.json();
        throw error;
    }
